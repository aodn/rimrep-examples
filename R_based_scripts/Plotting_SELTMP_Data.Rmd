...
1. Bar plot showing the proportion of recreational activities performed by people surveyed
...
```{r}
act_plot <- recreation %>% 
  #We use activity on x axis and ordering by order_plot in descending order
  ggplot(aes(x = reorder(activity, -order_plot), y = per_rec, fill = region))+
  #The dodge position will plot bars for each region next to each other
  geom_bar(position = "dodge", stat = "identity", color = "black")+
  geom_text(aes(label = per_rec), size = 2.25, hjust = -0.1,
  ...
  labs(y = "% respondents", title = "Waterway recreation activities performed by respondents")+
...
recreation %>% 
  ...
  geom_bar(position = "dodge", stat = "identity", color = "black")+
  facet_grid(~region)+
  geom_text(aes(label = per_rec), size = 2.25, hjust = -0.2,
