...
As we can see above, the ABS dataset contains `r nrow(data_df)` rows and `r ncol(data_df)` columns. However, this dataset should only include 4,355 rows, which means that it has been duplicated. We can remove any duplicate rows when loading the dataset into memory. We can further explore the contents of this dataset looking at its `schema`, which will print the name of all columns available in the dataset. Since there are 665 columns, we will just inspect the first 10 column names.
...
```{r subset_qld}
#Start a query
build_scan_query <- data_df$NewScan()
#Write query
build_scan_query$Filter(Expression$field_ref("REGION_CODE") >= 30000 & Expression$field_ref("REGION_CODE") <= 39999)
#End query
query <- build_scan_query$Finish()

#Apply query and load as data frame
qld_lgas <- query$ToTable() %>% 
  as.data.frame() %>% 
  #Removing duplicates 
  distinct(across(-geometry), .keep_all = T)

#Removing variables that are no longer needed
rm(build_scan_query, query)
...
townsville <- qld_lgas %>% 
  filter(REGION_NAME == "Townsville" & (year(TIME_PERIOD) == 2016 | year(TIME_PERIOD) == 2021)) %>% 
