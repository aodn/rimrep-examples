...
Before we continue subsetting the data, you may have noticed that when we inspected the dataset, all columns appear as character.

```{r}
head(qld_lgas)
```

We will change all columns included in the description table to numeric, which is the correct data type.
